...
class OutPuter {
	...
	public function page() {
		...
		// スレッドを取得
		$thread = $this->ci->this_model->getThread($this->q['thread_id'], $this->ci->session->user_id);
		$thread_url = '';
		$thread_tags = array();
		$comments = array();
		$base_id = null;
		...
		if (!empty($thread)) {
			...
			if ($thread->t_privated == 0) {
				...
				if (!empty($thread->t_comment_count)) {
					$comments = $this->ci->this_model->getComments($this->q['thread_id'], $base_id, $this->q['td'], $this->ci->session->user_id);
				}
			}
		} else {
		...
		if ($thread->t_privated == 0) {
			...
			$og_description = mb_substr($thread->t_body, 0, 150) . '...';
