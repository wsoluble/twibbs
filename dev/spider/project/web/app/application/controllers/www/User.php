		...
		if (!empty($user)) {
			switch ($this->q['method']) {
				case '':
					...
					if (!empty($record_max)) {
						$records = $this->ci->this_model->getCommentsFromUserID($user->user_id, $base_id, $this->q['td'], $this->ci->session->user_id);
					}
					break;
				...
				case 'thread':
					...
					if (!empty($record_max)) {
						$records = $this->ci->this_model->getThreadsFromUserID($user->user_id, $base_id, $this->q['td'], $this->ci->session->user_id);
					}
					break;
				...
				case 'bookmark':
					...
					if (!empty($record_max)) {
						$records = $this->ci->this_model->getFavoriteThreadsFromUserID($user->user_id, $base_id, $this->q['td'], $this->ci->session->user_id);
					}
					break;
		...
		if (!empty($user)) {
			...
			switch ($this->q['method']) {
				...
				case 'thread':
					...
					$temp->head_params['title'] .= ' - スレッド';
					break;

				case 'bookmark':
					$temp->directories[] = array('dir' => $user_url, 'title' => 'ぶくま！');
					$temp->head_params['title'] .= ' - ぶくま！';
					break;
