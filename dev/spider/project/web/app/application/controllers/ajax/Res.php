...
class Res extends CI_Controller {
	...
	public function add() {
		...
		if ($this->input->is_ajax_request()) {
			...
			// NGユーザー
			if ($this->this_model->isNgUser($thread->t_user_id, $_SESSION['user_id'])) {
	...
	public function remove() {
		...
		if ($this->input->is_ajax_request()) {
			...
			if (empty($err_msgs)) {
				// 削除
				$result = $this->this_model->removeComment($comment_id, $_SESSION['user_id']);
				if (!isset($result->successful)) {
					switch ($result->error) {
						case 'invalid_parameter':
							echo '<div id="ajax-error"><span class="error">パラメータが不正です</span></div>';
				...
				// コメント一覧を描画
				$comment = $this->this_model->getComment($comment_id, $_SESSION['user_id']);
				$comment_entry_data = array();
