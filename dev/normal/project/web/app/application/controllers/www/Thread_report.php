...
class Thread_report extends CI_Controller {
	...
	public function get($thread_id) {
		...
		$posted = false;
		if (!empty($_SESSION['user_id'])) {
			$posted = $this->this_model->postedReportThread($s_user_id, $thread_id);
		}
		// スレッドを取得
		$thread = $this->this_model->getThread($thread_id, $s_user_id);
		...
		if (!empty($thread->t_report_count)) {
			if ($this->session->twitter_primary_id == ADMIN_TWITTER_PRIMARY_ID || $thread->t_user_id == $s_user_id) {
		...
		$temp->content .= '<h2 class="report-title">' . $report_title . '</h2>' . "\n";
