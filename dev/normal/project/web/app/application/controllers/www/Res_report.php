...
class Res_report extends CI_Controller {
	...
	public function get($comment_id) {
		...
		$posted = false;
		if (!empty($_SESSION['user_id'])) {
			$posted = $this->this_model->postedReportComment($s_user_id, $comment_id);
		}
		// レスを取得
		...
		// スレッドを取得
		$thread = $this->this_model->getThread($comment->c_thread_id, $s_user_id);
		// 報告の種類
		...
		if (!empty($comment->c_report_count)) {
			if ($this->session->twitter_primary_id == ADMIN_TWITTER_PRIMARY_ID || $thread->t_user_id == $s_user_id || $comment->c_user_id == $s_user_id) {
				...
				if ($posted == true) {
					$reports = $this->this_model->getCommentReport($s_user_id, $comment_id);
		...
		$temp->content .= '<h1 class="report-title">' . $report_title . '</h1>' . "\n";
