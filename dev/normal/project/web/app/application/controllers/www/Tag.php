	...
	public function page() {
		...
		if (!empty($tag)) {
			...
			if (!empty($tag->tg_thread_count)) {
				$records = $this->ci->this_model->getThreadsWithTag($tag->tag_id, $base_id, $this->q['td'], $s_user_id);
			}
		}
		...
		$temp->head_params['description'] = '「' . $word . '」タグのスレッド';
		$temp->head_params['keywords'] = array($this->q['word'], 'スレッド');

		$temp->head_params['og'] = array();
		$temp->head_params['og']['type'] = 'article';
		$temp->head_params['og']['title'] = $word;
		$temp->head_params['og']['url'] = $tag_url;
		$temp->head_params['og']['description'] = '「' . $word . '」タグのスレッド';
