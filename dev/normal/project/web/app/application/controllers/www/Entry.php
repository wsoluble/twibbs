...
class OutPuter {
	...
	public function page() {
		...
		if (!empty($this->q['url_name'])) {
			$current_category = $temp->category = $this->ci->this_model->getCategoryFromUrlName($this->q['url_name']);
			if (empty($temp->category)) {
				$this->ci->lib_base->output_404();
			}
			if (empty($temp->category->cat_parent)) {
				$temp->p_category = $temp->category;
				$temp->category = null;
			} else {
		...
		if (empty($current_category)) {
			...
			$entry_url = '/entry';
			$temp->head_params['rss'] = '/rss/all.xml';
			$first_id = 1;
			// 範囲チェック
			...
			if (!empty($site->s_thread_count)) {
				$records = $this->ci->this_model->getThreads($base_id, $this->q['td'], $s_user_id);
			}
		} else {
			$temp->head_params['title'] = $current_category->cat_word;
			$entry_url = '/entry/' . $current_category->cat_url_name;
			$temp->head_params['rss'] = '/rss/' . $current_category->cat_url_name . '.xml';
			$first_id = $current_category->cat_first_thread_id;
			// 範囲チェック
			if (!empty($this->q['base'])) {
				$base_id = ($this->q['base'] < $current_category->cat_first_thread_id) ? $current_category->cat_first_thread_id : $this->q['base'];
				$base_id = ($this->q['base'] > $current_category->cat_last_thread_id) ? $current_category->cat_last_thread_id : $this->q['base'];
				if ($this->q['base'] >= $current_category->cat_last_thread_id && $this->q['td'] == 'after') {
					header('Location: /entry/' . $current_category->cat_url_name);
					exit();
				}
			}
			if (!empty($current_category->cat_thread_count)) {
				$records = $this->ci->this_model->getThreadsWithCategory($base_id, $this->q['td'], $current_category, $s_user_id);
			}
		}
		...
		$temp->dir_title = 'エントリ';
		$url = 'entry';
		$temp->head_params['keywords'] = array('エントリ', 'スレッド');
		$temp->head_params['description'] = 'すべてのスレッドの一覧。';
		if (!empty($temp->p_category)) {
			...
			$temp->head_params['keywords'] = array($temp->p_category->cat_word, 'スレッド');
			$temp->head_params['description'] = '「' . $temp->p_category->cat_word . '」カテゴリのスレッドの一覧。';
		}
		if (!empty($temp->category)) {
			...
			$temp->head_params['keywords'] = array($temp->category->cat_word, 'スレッド');
			$temp->head_params['description'] = '「' . $temp->category->cat_word . '」カテゴリのスレッドの一覧。';
		}
	...
	public function ajax() {
		...
		if (empty($category)) {
			...
			if (!empty($category->cat_thread_count)) {
				$records = $this->ci->this_model->getThreadsWithCategory($base_id, $this->q['td'], $category, $s_user_id);
			}
		}
		// afterの場合は順番を逆にする（新しい順に）
		if ($this->q['td'] == 'after') {
