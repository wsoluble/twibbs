...
$(document).ready(function(){
	...
	$(document).on('change', '#default-media-display', function() {
		if (check_ls()) {
			if ($(this).prop('checked')) {
				window.localStorage.setItem('default_media_display', 1);
			} else {
				window.localStorage.setItem('default_media_display', 0);
			}
		}
	});
	...
	$('.menu-toggle').sidr({
		...
		onOpen: function() {
			...
			$('html,body').css({"position":"fixed","top": -(scroll_y) + "px"});
			/*
		...
		onClose: function() {
			$('html,body').css({"position":"static","top": "auto"});
	...
	$(document).on('click', '.thread-about', function() {
		...
		$('.entry-header .body').slideToggle('normal', function() {
			if ($('.entry-header .body').is(':hidden')) {
		...
		if (body) {
			...
			$('#post-preview').fadeOut(function () {
				...
				if (conv.media != '') {
					...
					display_first_media($('#post-preview'));
				}
				$('#post-preview').fadeIn();
			});
		} else {
		...
		if (!loading) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('#content .comment').removeClass('res-selected');
					$(img).hide();
				},
				error: function(XMLHttpRequest, status, error) {
...
function conv_res_body(body, escape) {
	...
	ret.body = ret.body.replace(/@([a-zA-Z0-9_]{1,15})/g, function(match) {
		var twid = match.replace(/@/, '');
		return '<em class="jump-twitter" data-twid="' + twid + '">' + match + '</em>';
	});
	ret.body = ret.body.replace(/(>>|&gt;&gt;)([0-9]+)/g, function(match) {
		var cid = match.replace(/(>>|&gt;&gt;)/, '');
		return '<em class="jump-res" data-cid="' + cid + '">' + match + '</em>';
	});
	return ret;
}
...
function gen_media_content(button, target) {
	...
	switch (site) {
		...
		case 'niconico':
			...
			var src = 'http://ext.nicovideo.jp/thumb_watch/' + id + '?w=' + w + '&h=' + h;
...
function update_default_media_display() {
	if (check_ls()) {
		var checked = 0;
		if (window.localStorage.getItem('default_media_display') == null) {
			window.localStorage.setItem('default_media_display', 1);
		}
		checked = JSON.parse(window.localStorage.getItem('default_media_display'));
		if (checked) {
			$('#default-media-display').prop('checked', true);
		} else {
			$('#default-media-display').prop('checked', false);
		}
	}
}

function display_first_media(body) {
	if ($('#default-media-display').prop('checked') == true) {
		var first_button = $(body).find('.media-buttons button').eq(0);
		var target_view = $(body).find('.media-view');
		$(first_button).attr('selected', '');
		gen_media_content(first_button, target_view);
	}
}
