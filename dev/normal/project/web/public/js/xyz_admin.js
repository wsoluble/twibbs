$(document).ready(function() {
	...
	$(document).on('change', '#admin-thread-user-select', function() {
		var uid = $(this).val();
		var ad_user = null;
		for (var ukey in xyz_users) {
			if (xyz_users[ukey].user_id == uid) {
				ad_user = xyz_users[ukey];
				break;
			}
		}
		$('#admin-thread-user-name').attr('href', 'https://twitter.com/' + ad_user.u_twitter_id);
		$('#admin-thread-user-name img').attr('src', ad_user.u_twitter_profile_image);
	});

	$(document).on('change', '#admin-comment-user-select', function() {
		var uid = $(this).val();
		var ad_user = null;
		for (var ukey in xyz_users) {
			if (xyz_users[ukey].user_id == uid) {
		...
		$('#admin-comment-user-name').attr('href', 'https://twitter.com/' + ad_user.u_twitter_id);
		$('#admin-comment-user-name').html('@' + ad_user.u_twitter_id);
		$('#admin-comment-user-thumb').attr('src', ad_user.u_twitter_profile_image);
	});
